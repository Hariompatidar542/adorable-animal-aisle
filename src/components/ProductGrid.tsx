...
export const ProductGrid: React.FC<ProductGridProps> = ({
  ...
  return <section className="py-16 bg-white">
      <div className="container mx-auto px-4">
        ...
        }} className={`${activeCategory === category ? 'gradient-primary text-white' : 'hover:bg-muted/50'} transition-all duration-200`}>
              {category}
            </Button>)}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {displayedProducts.map((product, index) => <ProductCard key={product.id} product={product} onClick={() => onProductClick(product)} className={`animate-fade-in`} style={{
        ...
        {visibleProducts < filteredProducts.length && <div className="text-center">
            <Button variant="outline" size="lg" onClick={() => setVisibleProducts(prev => prev + 6)} className="hover:bg-muted/50">
              Load More Products
            </Button>
          </div>}
      </div>
